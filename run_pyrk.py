...
# Reactivity ramp rate (pcm per minute) - set to get approx 1%/min power ramp rate
RHO_RATE_PCM_PER_MIN = 2.0


@dataclass
class Scenario:
    p0: float          # label for initial "fraction of rated power"
...
def generate_scenarios():
    """
    For each (initial power label, bucket), create:
      - one 'up' ramp scenario
      - one 'down' ramp scenario
    Both use the same ramp duration and rho ramp rate.
    """
    scenarios = []
...
def initial_temps_kelvin(bucket: str):
    """
    Return (t_fuel, t_mod, t_shell, t_cool) in Kelvin (floats)
    for the chosen temperature bucket.

    - 'nominal' matches the original PB-FHR example input.
    - 'low' is a bit cooler than nominal.
    - 'high' is a bit hotter than nominal.
    """
    if bucket == "low":
...
def run_pyrk_case(run_dir: Path):
    plotdir = run_dir / "output"
        ...
        while len(procs) >= MAX_PARALLEL:
            ...
            for p, s in procs:
                ...
                if ret is None:
                    ...
                    print(f"[DONE] {s.run_name} (exit code {ret})")
            procs = still_running
            if len(procs) >= MAX_PARALLEL:
                time.sleep(30.0)

        print(f"[START] {scen.run_name}")
        proc = run_pyrk_case(run_dir)
        procs.append((proc, scen))
